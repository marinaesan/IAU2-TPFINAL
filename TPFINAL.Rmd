...
A continuación, analizo las emisiones de GEI originadas por la producción agrícola y de actividades asociadas al uso de la tierra a nivel regional (América del Sur).
...
La mayor parte de los países de la región, emiten semejantes cantidades de GEI, mientras que unos pocos superan los niveles promedio.
...
ggplot(arg_gei_3)+
geom_col(mapping = aes(x = factor (Año), y = Total_KT_CO2, fill = factor(Origen)))+ 
coord_flip()+
...
Analizando el origen de las emisiones, es posible
...
theme_minimal()+
coord_flip()+
facet_geo(~ Provincia, grid = argentina_grid2, scales= "free")+ 
...
geom_col(aes(x = factor(Año), y =Total_KT_CO2, fill=factor(Origen)), bins=50)+
facet_grid(Origen~., scales = 'free')
labs(title = "Niveles según fuente de origen", subtitle = "Período: 1990-2019",
     caption= "Fuente: https://www.fao.org/faostat/
     es/#data",x="Años", y ="", fill= "")+
theme(legend.position="none", axis.text.x = element_text(angle = 45, hjust = 1, size = 5),axis.text.y = element_text(size = 6), plot.margin = margin(0.25, 1, 0.25, 0.1, "cm"),plot.subtitle = element_text(size=7.5),plot.caption=element_text(size=5))
